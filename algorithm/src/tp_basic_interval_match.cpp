...
double var_na_zero(Ti begin, Ti end)
{
    ...
    std::size_t n = std::distance(begin, end);
    double mean = std::accumulate(begin, end, 0.0) / n;
    double var = std::accumulate(begin, end, 0.0, [&](const auto r, const auto v)
                                 { return r + (v - mean) * (v - mean); }) /
                 (n - 1);
    return var;
}
...
// Time weighted summary functions
                                ...
                                int minThreads = 1)
{
    ...
    // the scope for the thread pool
    {
        ...
        // loop goes from 1..n, not 0..n-1, as we're looking for ENDS of groups
        for (auto i = 1u; i <= n; ++i)
        {
            bool end_of_data = (i == n);
            bool group_changed = (!end_of_data && (groups[i] != this_group));

            // aggregate if we're at the end of the data or the group changed
            if (end_of_data || group_changed)
            {
                ...
                in_intervals = false;
                std::size_t j_start = interval_group_end; // continue from last end
                for (std::size_t j = j_start; j < n_int; ++j)
                {
                    if (!in_intervals && int_groups[j] == this_group)
                    {
                        in_intervals = true;
                        interval_group_start = j;
                        interval_group_end = n_int; // provisional
                    }
                    if (in_intervals && int_groups[j] != this_group)
                    {
                        interval_group_end = j; // close range
                        break;
                    }
                }
                ...
                // start next group (only if there *is* a next element)
                if (!end_of_data)
                {
                    this_group = groups[i];
                    this_group_start = i;
...
public:
    interval_summarize(Tg *groups, Td *times, Tv *values, Tg *interval_groups, Td *interval_begin, Td *interval_end, Tv *res, std::size_t group_start, std::size_t group_size, std::size_t interval_group_start, std::size_t interval_group_end, int fun_aggregate, double locf = 0.0, double nocb = 0.0)
    {
        // copy the data pointers
        this->groups = groups;
        this->times = times;
                                           ...
                                           int minThreads = 1)
{
    ...
    // return immediately if there are size 0 data vectors
    if (n == 0 || n_int == 0)
    {
        return res;
    }
...
// a overlap aggregation function that allows intervals to overlap on other intervals
// especially useful for checking if a sedation period overlaps an interval
template <typename Td, typename Tg, typename Tv>
class interval_interval_overlap_aggregate
{
private:
    // the data vectors
    Tg *groups;
...
public:
    interval_interval_overlap_aggregate(Tg *groups,
                                        Td *times_begin,
                                        Td *times_end,
                                        Tv *values,
                                        Tg *interval_groups,
                                        ...
                                        double nocb = 0.0)
    {
        ...
        this->res = res;
        this->res_time_begin = res_time_begin;
        this->res_time_end = res_time_end;
        // this->res_time_difference = res_time;
        ...
        Rcpp::Rcout << "group_start: " << group_start << " group end: " << group_end << "\n";
    }
    ...
    auto operator()() -> void
    {
        ...
        typedef typename decltype(typename std::remove_pointer<decltype(times_begin)>::type())::value_type TIMES;
        TIMES locf_max = static_cast<TIMES>(locf);
        TIMES nocb_max = static_cast<TIMES>(nocb);
        // static_cast<TIMES>
        // std::cout << "locf_max: " << locf_max << "\n";
        ...
        for (auto i = interval_group_start; i < interval_group_end; ++i)
        {
            ...
            for (auto j = last_element_before; j < group_end; ++j)
            {
                ++nloops;
                // update the last element before the interval start, i.e. possible locf-value
                if ((*times_end)[j] < (*interval_begin)[i])
                {
                    last_element_before = j;
                }
                else if (!matching_elements_found && ((*times_end)[j] >= (*interval_begin)[i] && (*times_begin)[j] <= (*interval_end)[i]))
                {
                    // we match when the times_end is greater or equal than interval_begin and times_begin is less or equal than interval_end
                    matching_elements_found = true;
                    aggregate_begin = values->begin() + j;
                }
                // break the loop if the current begin time is larger than the current interval end or we're at the end of the group
                if (((*times_begin)[j] > (*interval_end)[i]) || j == group_end - 1)
                {
                    // call aggregation function, if we've previously found elements
                    if (matching_elements_found)
                    {
                        ...
                        aggregate_end = values->begin() + j;
                        // make sure we include the last value when it is also within aggregation
                        if ((j == (group_end - 1)) && (((*times_end)[j] >= (*interval_begin)[i]) && ((*times_begin)[j] <= (*interval_end)[i])))
                        {
                            ++aggregate_end;
                        }
                        ...
                        if (target_position != values->end())
                        {
                            (*res)[i] = (*target_position);
                            auto t_index = std::abs(std::distance(values->begin(), target_position));
                            (*res_time_begin)[i] = (*times_begin)[t_index];
                            (*res_time_end)[i] = (*times_end)[t_index];
                            // we are in the interval, so we miss by 0
                            //(*res_time_difference)[i] = 0;
                            result_set = true;
                        }
                    ...
                    // if we haven't set any result yet, possibly set result using locf
                    if (!result_set && ((*times_end)[last_element_before] < (*interval_begin)[i]) && ((*times_end)[last_element_before] + locf_max >= (*interval_begin)[i]))
                    {
                        (*res)[i] = (*values)[last_element_before];
                        (*res_time_begin)[i] = (*times_begin)[last_element_before];
                        (*res_time_end)[i] = (*times_end)[last_element_before];
                        result_set = true;
                    }

                    // if that doesn't work, try nocb
                    if (!result_set && (*times_begin)[j] > (*interval_end)[i] && (*times_begin)[j] - nocb_max <= (*interval_end)[i])
                    {
                        (*res)[i] = (*values)[j];
                        (*res_time_begin)[i] = (*times_begin)[j];
                        (*res_time_end)[i] = (*times_end)[j];
                        result_set = true;
                    }
...
// a overlap aggregation function that allows intervals to overlap on other intervals, not
// only time points to overlap over intervals.
//[[Rcpp::export]]
Rcpp::DataFrame cpp_interval_interval_aggregate(Rcpp::NumericVector groups,
                                                ...
                                                int minThreads = 1)
{
    ...
    // the out vector, and the vector of futures
    Rcpp::NumericVector res(n_int, Rcpp::NumericVector::get_na());
    Rcpp::NumericVector res_time_begin(n_int, Rcpp::NumericVector::get_na());
    Rcpp::NumericVector res_time_end(n_int, Rcpp::NumericVector::get_na());

    if (n == 0 || n_int == 0)
    {
        return Rcpp::DataFrame::create(
            Rcpp::Named("begin") = res_time_begin,
            Rcpp::Named("end") = res_time_end,
            Rcpp::Named("value") = res);
    }
    // Rcpp::NumericVector res_time_difference(n_int, Rcpp::NumericVector::get_na());
    std::vector<std::future<void>> resvec;
            ...
            if (end_of_data || group_changed)
            {
                ...
                if (in_intervals)
                {
                    resvec.push_back(std::move(pool.add_task(
                        ...
                        interval_interval_overlap_aggregate<Rcpp::NumericVector, Rcpp::NumericVector, Rcpp::NumericVector>(&groups,
                                                                                                                           &times_begin,
                                                                                                                           &times_end,
                                                                                                                           &values,
                                                                                                                           &int_groups,
    ...
    Rcpp::DataFrame out = Rcpp::DataFrame::create(Rcpp::Named("begin") = res_time_begin, Rcpp::Named("end") = res_time_end, Rcpp::Named("value") = res);
...
// especially useful for checking if a sedation period overlaps an interval
template <typename Td, typename Tg, typename Tv>
class interval_interval_overlap_aggregate_double
{
private:
    // the data vectors
    Tg *groups;
...
public:
    interval_interval_overlap_aggregate_double(Tg *groups,
                                               Td *times_begin,
                                               Td *times_end,
                                               Tv *values,
                                               Tv *value_times,
    ...
    auto operator()() -> void
    {
        ...
        for (auto i = interval_group_start; i < interval_group_end; ++i)
        {
            ...
            for (auto j = last_element_before; j < group_end; ++j)
            {
                ...
                {
                    last_element_before = j;
                    //Rcpp::Rcout << "Setting locf element to " << j << "\n";
                }
                else if (!matching_elements_found && ((*times_end)[j] >= (*interval_begin)[i] && (*times_begin)[j] <= (*interval_end)[i]))
                {
                    // we match when the times_end is greater or equal than interval_begin and times_begin is less or equal than interval_end
                    matching_elements_found = true;
                    aggregate_begin = values->begin() + j;
                    //Rcpp::Rcout << "found a match at " << j << "\n";
                }
                // break the loop if the current begin time is larger than the current interval end or we're at the end of the group
                if (((*times_begin)[j] > (*interval_end)[i]) || j == group_end - 1)
                {
                    //Rcpp::Rcout << "Breaking loop...\n";
                    // call aggregation function, if we've previously found elements
                    if (matching_elements_found)
                    {
                        //Rcpp::Rcout << "matching element found...\n";
                        // the end iterator is either j, j+1, depending on the condition for break
                        aggregate_end = values->begin() + j;
                        ...
                        //Rcpp::Rcout << "aggregate_begin: " << aggregate_begin << " aggregate_end: " << aggregate_end << "\n";
                        ...
                        if (target_position != values->end())
                        {
                            (*res)[i] = (*target_position);
                            auto t_index = std::abs(std::distance(values->begin(), target_position));
                            (*res_times)[i] = (*value_times)[t_index];
                            (*res_time_begin)[i] = (*times_begin)[t_index];
                            (*res_time_end)[i] = (*times_end)[t_index];
                            // we are in the interval, so we miss by 0
                            //(*res_time_difference)[i] = 0;
                    ...
                    {
                        (*res)[i] = (*values)[last_element_before];
                        (*res_times)[i] = (*value_times)[last_element_before];
                        (*res_time_begin)[i] = (*times_begin)[last_element_before];
                        (*res_time_end)[i] = (*times_end)[last_element_before];
                        result_set = true;
                    }
...
// only time points to overlap over intervals.
//[[Rcpp::export]]
Rcpp::DataFrame cpp_interval_interval_aggregate_double(Rcpp::NumericVector groups,
                                                       Rcpp::NumericVector times_begin,
                                                       Rcpp::NumericVector times_end,
                                                       Rcpp::NumericVector values,
                                                       Rcpp::NumericVector value_times,
                                                       Rcpp::NumericVector int_groups,
                                                       Rcpp::NumericVector int_begin,
                                                       Rcpp::NumericVector int_end,
                                                       Rcpp::String aggregation_function = "min",
                                                       ...
                                                       int minThreads = 1)
{
    ...
    // the out vector, and the vector of futures
    Rcpp::NumericVector res(n_int, Rcpp::NumericVector::get_na());
    Rcpp::NumericVector res_times(n_int, Rcpp::NumericVector::get_na());
    Rcpp::NumericVector res_time_begin(n_int, Rcpp::NumericVector::get_na());
    Rcpp::NumericVector res_time_end(n_int, Rcpp::NumericVector::get_na());
    ...
    if (n == 0 || n_int == 0)
    {
        return Rcpp::DataFrame::create(
            ...
            Rcpp::Named("end") = res_time_end,
            Rcpp::Named("value") = res,
            Rcpp::Named("value_time") = res_times);
    }
            ...
            if (end_of_data || group_changed)
            {
                ...
                if (in_intervals)
                {
                    resvec.push_back(std::move(pool.add_task(
                        ...
                        interval_interval_overlap_aggregate_double<Rcpp::NumericVector, Rcpp::NumericVector, Rcpp::NumericVector>(&groups,
                                                                                                                                  &times_begin,
                                                                                                                                  &times_end,
                                                                                                                                  &values,
                                                                                                                                  &value_times,
    ...
    Rcpp::DataFrame out = Rcpp::DataFrame::create(Rcpp::Named("begin") = res_time_begin, Rcpp::Named("end") = res_time_end, Rcpp::Named("value") = res, Rcpp::Named("value_time") = res_times);
