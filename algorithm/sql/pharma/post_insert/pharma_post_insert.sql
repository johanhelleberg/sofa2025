...
PRAGMA table_info('pharma_orders');				  
...
INSERT INTO pharma SELECT
	...
	ROW_NUMBER() 
		...
		AS RowNumber,
	GivenDose
	FROM (SELECT DISTINCT OrderNumber, CompID, PharmaID, DateTime, GivenDose FROM pharma_tmp);
...
CREATE VIEW 'hv_pharma_noninfusion' AS
	SELECT 
		PatientID, 
		pharma.CompID,
		CompName,
...
CREATE VIEW 'hv_pharma_infusion_fast' AS 
	SELECT
		po.PatientID,
		pi.OrderNumber,
		pi.RowNumber,
